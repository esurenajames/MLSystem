    ...
    UserTable = $('#example1').DataTable({
      ...
      "columns": [  { data: "ReferenceNo" }
                    ...
                    },
                    {
                      data: "StatusId", "render": function (data, type, row) {
                      if(row.StatusId == 1){
                          return '<a onclick="confirm(\'Are you sure you want to deactivate this Initial Capital?\', \''+row.CapitalId+'\', 0)" class="btn btn-danger" title="Deactivate"><span class="fa fa-close"></span></a> <a onclick="Edit('+row.CapitalId+')" data-toggle="modal" data-target="#modalNewRepaymentCycle" class="btn btn-success" title="Set Primary"><span class="fa fa-check-circle"></span></a>';
                        }
                        else if(row.StatusId == 0){
