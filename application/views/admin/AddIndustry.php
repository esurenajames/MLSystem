<div class="content-wrapper">
  ...
  <?php if(in_array('26', $subModule)) { ?>
    ...
    <div class="modal fade" id="modalNewIndustry">
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                      ...
                      <input type="hidden" class="form-control" id="txtIndustryId" name="IndustryId">
                    </div>
                  </div>
                  <div class="col-md-12">
                    <div class="form-group">
    ...
    <section class="content">
      <div class="box">
        ...
        <div class="box-body">
          <button type="button" class="btn btn-primary pull-right" data-toggle="modal" data-target="#modalNewIndustry">Add Industry</button>
          <br>
          <br>
...
<script>
  ...
  function confirm(Text, IndustryIdy, updateType)
  { 
    swal({
      title: 'Confirm',
      text: Text,
      ...
      $.ajax({                
          ...
          success: function(data)
          {
            ...
            swal({
              title: 'Success!',
              text: 'Industry Status successfully updated!',
              type: 'success',
              buttonsStyling: false,
              confirmButtonClass: 'btn btn-primary'
            });
  ...
  function Edit(IndustryId)
  {
    $.ajax({
      url: '<?php echo base_url()?>' + "/admin_controller/getIndustryDetails",
      type: "POST",
      async: false,
      data: {
        Id : IndustryId
      ...
      success: function(data)
      {
        $('#txtIndustry').val(data['Name']);
        $('#txtDescription').val(data['Description']);
        $('#txtIndustryId').val(IndustryId);
        $('#txtFormType').val(2);
        $('.loading').hide();
      },
  ...
  function refreshPage(){
    var url = '<?php echo base_url()."datatables_controller/Industry/"; ?>';
    ...
    UserTable = $('#example1').DataTable({
      ...
      "columns": [  { data: "ReferenceNo" }
                    ...
                    { data: "DateUpdated" }, 
                    {
                      data: "StatusId", "render": function (data, type, row) {
                      if(row.StatusId == 1){
                          return '<a onclick="confirm(\'Are you sure you want to deactivate this Industry?\', \''+row.IndustryId+'\', 0)" class="btn btn-danger" title="Deactivate"><span class="fa fa-close"></span></a> <a onclick="Edit('+row.IndustryId+')" data-toggle="modal" data-target="#modalNewIndustry" class="btn btn-info" title="Edit"><span class="fa fa-edit"></span></a>';
                        }
                        else if(row.StatusId == 0){
                          return '<a onclick="confirm(\'Are you sure you want to re-activate this Industry?\', \''+row.IndustryId+'\', 1)" class="btn btn-warning" title="Deactivate"><span class="fa fa-refresh"></span></a>';
                        }
                        else{
                          return "N/A";
                        }
