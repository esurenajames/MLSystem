...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li><a href="#">Types of Expenses</a></li>
    </ol>
  </section>


  <div class="modal fade" id="modalNewExpenseType">
        ...
        <form action="<?php echo base_url(); ?>admin_controller/AddExpenseType/" id="frmInsert2" method="post">
          <div class="modal-body">
              <div class="row">
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    <label for="Disbursement">Description</label><br>
...
<script>
  ...
  function confirm(Text, ExpenseTypeId, updateType)
  { 
    swal({
      title: 'Confirm',
      text: Text,
      ...
      $.ajax({                
          ...
          success: function(data)
          {
            ...
            swal({
              title: 'Success!',
              text: 'Expense Type successfully updated!',
              type: 'success',
              buttonsStyling: false,
              confirmButtonClass: 'btn btn-primary'
            });
  ...
  function Edit(ExpenseTypeId)
  {
    $.ajax({
      url: '<?php echo base_url()?>' + "/admin_controller/getExpenseTypeDetails",
      type: "POST",
      async: false,
      data: {
        Id : ExpenseTypeId
      ...
      success: function(data)
      {
        $('#txtExpenseType').val(data['ExpenseType']);
        $('#txtDescription').val(data['Description']);
        $('#txtExpenseTypeId').val(ExpenseTypeId);
        $('#txtFormType').val(2);
      },
  ...
  function refreshPage(){
    var url = '<?php echo base_url()."datatables_controller/ExpenseTypes/"; ?>';
  ...
  $(function () {
    UserTable = $('#example1').DataTable({
      "pageLength": 10,
      "ajax": { url: '<?php echo base_url()."/datatables_controller/ExpenseTypes/"; ?>', type: 'POST', "dataSrc": "" },
      "columns": [  { data: "ReferenceNo" }
                    ...
                    },
                    {
                      data: "StatusId", "render": function (data, type, row) {
                      if(row.StatusId == 1){
                          return '<a onclick="confirm(\'Are you sure you want to deactivate this Type of Expense?\', \''+row.ExpenseTypeId+'\', 0)" class="btn btn-danger" title="Deactivate"><span class="fa fa-close"></span></a> <a onclick="Edit('+row.ExpenseTypeId+')" data-toggle="modal" data-target="#modalNewExpenseType" class="btn btn-primary" title="Edit"><span class="fa fa-edit"></span></a>';
                        }
                        else if(row.StatusId == 0){
                          return '<a onclick="confirm(\'Are you sure you want to re-activate this Type of Expense?\', \''+row.ExpenseTypeId+'\', 1)" class="btn btn-warning" title="Deactivate"><span class="fa fa-refresh"></span></a>';
                        }
                        else{
                          return "N/A";
                        }
