    ...
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Loan Type Details</h4>
        </div>
...
<script>
  ...
  function confirm(Text, LoanId, updateType)
  { 
    swal({
      title: 'Confirm',
      text: Text,
  ...
  {
    $.ajax({
      url: '<?php echo base_url()?>' + "/admin_controller/getLoanTypeDetails",
      type: "POST",
      async: false,
      data: {
        Id : LoanId
  ...
  function refreshPage(){
    var url = '<?php echo base_url()."datatables_controller/Loans/"; ?>';
  ...
  $(function () {
    UserTable = $('#example1').DataTable({
      "pageLength": 10,
      "ajax": { url: '<?php echo base_url()."/datatables_controller/Loans/"; ?>', type: 'POST', "dataSrc": "" },
      "columns": [  { data: "LoanName" }
                    , { data: "Description" }
                    , {
    ...
    $('#modalNewLoanType').on('hide.bs.modal', function () {
      $('#txtFormType').val(1)
    })
